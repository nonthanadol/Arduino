...
#define INTERVAL_MESSAGE2 2000 //2วินาที
...
void loop() {
    if(millis() - time_1 > INTERVAL_MESSAGE1){ //หากเวลาไม่ถึงจำนวนที่ตั้งไว้ ก็จะข้ามไปทำอย่างอื่นก่อนได้เลย
        time_1 = millis();
        print_time(time_1);
        Serial.println("I'm message number one!");
    }
   
    if(millis() - time_2 > INTERVAL_MESSAGE2){ //ทำอันนี้ก่อน 
        time_2 = millis();
        print_time(time_2);
